...
/**
 *   	\file       bonretourline_list.php
 *		\ingroup    bonretour
 *		\brief      List page for bonretourline
 */
...
require_once __DIR__.'/class/bonretourline.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('bonretour', 'bonretourline', 'write');
	$permissiontodelete = $user->hasRight('bonretour', 'bonretourline', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("bonretour")) {
	accessforbidden('Module bonretour not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'BonretourLine';
	$objectlabel = 'BonretourLine';
	$uploaddir = $conf->bonretour->dir_output;
...
$title = $langs->trans("BonretourLines");
//$help_url = "EN:Module_BonretourLine|FR:Module_BonretourLine_FR|ES:Módulo_BonretourLine";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/bonretour/bonretourline_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-bonretour page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/bonretour/bonretourline_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendBonretourLineRef";
$modelmail = "bonretourline";
$objecttmp = new BonretourLine($db);
